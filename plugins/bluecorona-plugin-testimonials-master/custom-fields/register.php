...
$city = get_post_meta( $post->ID, 'testimonial_city', true );
$state = get_post_meta( $post->ID, 'testimonial_state', true );
...
<div class="container">
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-4">
      <input type="text" class="form-control" name="testimonial_city" id="testimonial_city" value="<?= $city?>" >
    </div>
    <label class="col-sm-2 col-form-label">State</label>
    <div class="col-sm-4">
      <input type="text" class="form-control" name="testimonial_state" id="testimonial_state" value="<?= $state?>" >
    </div>
  </div>
<!--   <div class="form-group row">
    <label class="col-sm-2 col-form-label">Heading</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" name="testimonial_heading" id="testimonial_heading" value="<?= $heading?>" >
    </div>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
        <input type="text" name="testimonial_custom_image" id="" class="meta-image col-sm-2" value="<?= $image;?>"  accept='image/*' style="margin-top: 40px;">
...
function bc_testimonial_save_metabox( $post_id, $post ) {
    ...
    //     return $post->ID;
    // }
      if ( !isset( $_POST['testimonial_city'] ) ) {
        return $post->ID;
    }
      if ( !isset( $_POST['testimonial_state'] ) ) {
        return $post->ID;
    }
    $sanitizedname = wp_filter_post_kses( $_POST['testimonial_name'] );
    // $sanitizedtitle = wp_filter_post_kses( $_POST['testimonial_title'] );
    // $sanitizedheading = wp_filter_post_kses( $_POST['testimonial_heading'] );
    $sanitizedmessage = wp_filter_post_kses( $_POST['testimonial_message'] );
    $sanitizedcity = wp_filter_post_kses( $_POST['testimonial_city'] );
    $sanitizedstate = wp_filter_post_kses( $_POST['testimonial_state'] );
    ...
    // update_post_meta( $post->ID, 'testimonial_heading', $sanitizedheading );
    update_post_meta( $post->ID, 'testimonial_message', $sanitizedmessage );

    update_post_meta( $post->ID, 'testimonial_city', $sanitizedcity );
    update_post_meta( $post->ID, 'testimonial_state', $sanitizedstate );
